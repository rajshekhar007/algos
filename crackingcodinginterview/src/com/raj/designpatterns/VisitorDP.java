...
public class VisitorDP {
    ...
    public Element[] list = {new This(), new That(), new TheOther()};

    // 4. Client creates "visitor" objects and passes each to accept() calls
    public static void main(String[] args) {
        VisitorDP visitorDP = new VisitorDP();
        UpVisitor up = visitorDP.new UpVisitor();
        DownVisitor down = visitorDP.new DownVisitor();
        for (int i = 0; i < visitorDP.list.length; i++) {
            visitorDP.list[i].accept(up);
        }
        for (int i = 0; i < visitorDP.list.length; i++) {
            visitorDP.list[i].accept(down);
        }
    }
